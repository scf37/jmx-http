...
echo "$PWD/$(dirname "$0")"/target/jmx-http-client-urlconnection-${PROJECT_VERSION}.jar
#jvisualvm --cp:p "$PWD/$(dirname "$0")"/target/jmx-http-client-urlconnection-${PROJECT_VERSION}.jar
HOME=/home/asm/dev/jdk1.8.0_181/lib
 jconsole -J-Djava.class.path=$HOME/jconsole.jar:$HOME/tools.jar:./target/jmx-http-client-urlconnection-1.0.0-SNAPSHOT.jar
